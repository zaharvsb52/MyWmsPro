...
namespace wmsMLC.DCL.General.Helpers
{
    public class Mapi
    {
        static public void SendMail(String aTo, String aCC, String aSubject, String aBody, String[] aAttachments, Boolean aShowDialog)
        {
            Mapi ma = new Mapi();
            ...
            if (aTo.Length > 0)
            {
                String[] tmpRecips = aTo.Split(new String[] { ";" }, StringSplitOptions.RemoveEmptyEntries);
                foreach (String tmpRecip in tmpRecips)
                    ma.AddRecip(tmpRecip.Replace(" ", ""), null, false);
                //ma.AddRecip(aTo, null, false);
            }

            if (aCC.Length > 0)
            {
                String[] tmpRecips = aCC.Split(new String[] { ";" }, StringSplitOptions.RemoveEmptyEntries);
                foreach (String tmpRecip in tmpRecips)
                    ma.AddRecip(tmpRecip.Replace(" ", ""), null, true);
            }
            ...
            if ((aAttachments != null) && (aAttachments.Length > 0))
            {
                ...
                {
                    FileInfo fi = new FileInfo(aAttachments[i]);
                    ma.Attach(fi.FullName);
                }
            }

            try
            {
                //Проверяем, что перед нами XP
                if (Environment.OSVersion.Version.Major < 6)
                ma.Logon(IntPtr.Zero);

                if (!ma.Send(aSubject, aBody, aShowDialog))
                {
                    //throw new Exception(string.Format(ResourceHelper.GetString("wmsMLC.DCL.Resources:ExceptionResources:CantSendEmail"), ma.Error())); //"Can't send e-mail ({0})."
                    throw new Exception(string.Format(ExceptionResources.CantSendEmail, ma.Error()));
                }
            }
            finally
            {
        ...
        public bool Send(string sub, string txt, Boolean ShowDialog)
        {
